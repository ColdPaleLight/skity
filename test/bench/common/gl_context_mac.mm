...
class GLContextDarwin : public GLContext {
 public:
  GLContextDarwin();
...
GLContextDarwin::GLContextDarwin() {
...
bool GLContextDarwin::MakeCurrent() {
  if (!context_) {
    return false;
  }
...
bool GLContextDarwin::ClearCurrent() {
  [NSOpenGLContext clearCurrentContext];
  return true;
}

bool GLContextDarwin::IsCurrent() { return [NSOpenGLContext currentContext] == context_; }
void *GetGLProcLoader() { return (void *)GetProcLoader; }

std::shared_ptr<GLContext> CreateGLContextMac() { return std::make_shared<GLContextDarwin>(); }
