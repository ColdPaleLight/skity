...
std::string WGXAdvancedBlending::GenSourceWGSL() const {
  std::stringstream ss;
...
void WGXAdvancedBlending::SetupBindGroup(Command* cmd, HWDrawContext* context) {
  if (dst_read_strategy_ != DstReadStrategy::kTextureCopy) {
    return;
  }
  ...
  auto group = cmd->pipeline->GetBindingGroup(2);
  if (group == nullptr) {
    return;
  }

  DEBUG_CHECK(context->prev_draw_pass);
  auto* prev_draw_pass = context->prev_draw_pass;

  // uv mapping
  {
    auto uv_mapping_entry = group->GetEntry(0);

    if (uv_mapping_entry == nullptr ||
        uv_mapping_entry->type_definition->name != "vec4<f32>") {
      return;
    }
    auto dst_uv_mapping = prev_draw_pass->dst_uv_mapping;
  ...
  UploadBindGroup(group->group, sampler_binding, cmd,
                  prev_draw_pass->dst_sampler);
  UploadBindGroup(group->group, texture_binding, cmd,
                  prev_draw_pass->dst_texture);
}
